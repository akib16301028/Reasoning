        ...
        if not all(col in df.columns for col in required_columns):
            ...
            with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                for day in last_3_days:
                    ...
                    # Write data to Excel
                    day_power.to_excel(writer, sheet_name=f"Power_{day_str}", index=False)
                    day_outage.to_excel(writer, sheet_name=f"Outage_{day_str}", index=False)
                    
                    # Create Excel chart in a new sheet
                    workbook = writer.book
                    chart_sheet = workbook.create_sheet(title=f"Chart_{day_str}")
                    
                    # Add the image to Excel
                    img = ExcelImage(img_buffer)
                    chart_sheet.add_image(img, 'A1')
                    
                    # Also create native Excel charts
                    self.create_excel_charts(workbook, day_power, day_outage, day_str)
            ...
            st.download_button(
                ...
                data=excel_buffer,
                file_name="power_system_analysis_with_charts.xlsx",
...
def create_excel_charts(self, workbook, day_power, day_outage, day_str):
    """Create native Excel charts in the workbook"""
    # Create a new sheet for Excel native charts
    chart_sheet = workbook.create_sheet(title=f"ExcelChart_{day_str}")
    
    # Write data to the sheet
    chart_sheet.append(["Hour"] + list(day_power.columns[1:]) + ["Outage Count"])
    for hour in range(24):
        row = [hour]
        for event in day_power.columns[1:]:
            row.append(day_power[event][hour] if hour in day_power['Hour'].values else 0)
        
        # Add outage count
        outage_count = 0
        if not day_outage.empty and hour in day_outage['Hour'].values:
            outage_count = day_outage[day_outage['Hour'] == hour]['Count'].values[0]
        row.append(outage_count)
        
        chart_sheet.append(row)
    
    # Create bar chart for power events
    bar_chart = BarChart()
    bar_chart.type = "col"
    bar_chart.style = 10
    bar_chart.title = f"Power Events Duration - {day_str}"
    bar_chart.y_axis.title = 'Duration (Hours)'
    bar_chart.x_axis.title = 'Hour of Day'
    
    data = Reference(chart_sheet, min_col=2, min_row=1, max_col=len(day_power.columns), max_row=25)
    cats = Reference(chart_sheet, min_col=1, min_row=2, max_row=25)
    bar_chart.add_data(data, titles_from_data=True)
    bar_chart.set_categories(cats)
    bar_chart.shape = 4
    chart_sheet.add_chart(bar_chart, "A30")
    
    # Create line chart for outage events
    line_chart = LineChart()
    line_chart.title = f"Outage Events - {day_str}"
    line_chart.y_axis.title = 'Count'
    line_chart.style = 12
    
    data = Reference(chart_sheet, min_col=len(day_power.columns)+1, min_row=1, max_col=len(day_power.columns)+1, max_row=25)
    line_chart.add_data(data, titles_from_data=True)
    line_chart.set_categories(cats)
    chart_sheet.add_chart(line_chart, "J30")
    
    # Create combo chart
    combo_chart = BarChart()
    combo_chart.type = "col"
    combo_chart.style = 10
    combo_chart.title = f"Combined Analysis - {day_str}"
    combo_chart.y_axis.title = 'Duration (Hours)'
    
    # Add power events
    power_data = Reference(chart_sheet, min_col=2, min_row=1, max_col=len(day_power.columns), max_row=25)
    combo_chart.add_data(power_data, titles_from_data=True)
    combo_chart.set_categories(cats)
    
    # Add outage events as line on secondary axis
    line = LineChart()
    outage_data = Reference(chart_sheet, min_col=len(day_power.columns)+1, min_row=1, max_col=len(day_power.columns)+1, max_row=25)
    line.add_data(outage_data, titles_from_data=True)
    combo_chart.y_axis.crosses = "max"
    combo_chart += line
    
    chart_sheet.add_chart(combo_chart, "A50")
